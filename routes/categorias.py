...
from utils.Validaciones import es_solo_letras, tiene_caracteres_especiales, limite_caracteres, longitud_valida
...
def create_categoria():
    try:
        ...
        if not valores_categorias['nombre']:
            return respuesta_json_fail('El nombre de la categoría no puede estar vacío.', 400)
        
        if not es_solo_letras(valores_categorias['nombre']):
            return respuesta_json_fail('El nombre de la categoría solo puede contener letras.', 400)
        
        if tiene_caracteres_especiales(valores_categorias['nombre']):
            return respuesta_json_fail('El nombre de la categoría no puede contener caracteres especiales.', 400)

        if not limite_caracteres(valores_categorias['nombre'], 30):
            return respuesta_json_fail('El nombre de la categoría no puede tener más de 30 caracteres.', 400)
        ...
        return respuesta_json_success({'mensaje': 'Categoría ingresada exitosamente'},200)
